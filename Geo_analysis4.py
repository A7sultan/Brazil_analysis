...
agg_name = 'geolocation_zip_code_prefix'
a= atibaia[agg_name].describe().to_frame()
...
@app.route('/')
def index():
    plot_html = create_plotly_map(atibaia, 'Map of Zip Codes in Atibaia State')
    return render_template('map4.html', a=a, plot_html=plot_html)
